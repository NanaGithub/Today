...
public class CalendarView extends ViewPager {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < ta.getIndexCount(); i++) {
            ...
            } else if (attr == R.styleable.CalendarView_today_bg) {
                mAttrsBean.setTodayBg(ta.getResourceId(attr, mAttrsBean.getTodayBg()));
            } else if (attr == R.styleable.CalendarView_other_bg) {
                mAttrsBean.setOtherBg(ta.getResourceId(attr, mAttrsBean.getOtherBg()));
    ...
    public void init() {
        ...
        addOnPageChangeListener(new SimpleOnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                refreshMonthView(position, date);
     ...
     * 刷新特殊日期
     */
    public void refreshSpecifyDate() {
        ...
        if (mAttrsBean.getSpecifyDate() != null && mAttrsBean.getSpecifyDate().size() > 0) {
            ...
            monthView.specifyChooseRefresh(hashSet);
        }
    }
     ...
     * @param date
     */
    public void refreshMonthView(int position, int[] date) {
        ...
        if (mAttrsBean.getInitDate() != null
                ...
                && mAttrsBean.getInitDate()[0] == date[0]
                && mAttrsBean.getInitDate()[1] == date[1]) {
            monthView.todayRefresh(mAttrsBean.getInitDate()[2]);
        }
        if (mAttrsBean.getChooseType() == 1) {//多选
            if (chooseDate.get(position) != null)
                monthView.multiChooseRefresh(chooseDate.get(position));
        } else {
            //单选时，如果设置切换月份不选中上次选中的日期 但如果切换回有选中日期的页则需要刷新选中，或者切换选中开启则需要刷新选中
    ...
    public void today() {
        ...
        int[] today = new int[]{CalendarUtil.getCurrentDate()[0], CalendarUtil.getCurrentDate()[1]};
        if (destPosition == currentPosition) {
            refreshMonthView(destPosition, today);
        } else {
    ...
    private void toDestDate(int year, int month, int day) {
        ...
        int[] desDate = new int[]{year, month};
        if (destPosition == currentPosition) {
            //在当月进行日期跳转
            refreshMonthView(destPosition, desDate);
        } else {
            setCurrentItem(destPosition, false);
        }
    }
    ...
    public CalendarView setInitDate(String date) {
        ...
        mAttrsBean.setInitDate(this.initDate);
        return this;
    }
     ...
     * @return
     */
    public CalendarView setSpecifyDate(List<String> dates) {
        List<int[]> multiDates = new ArrayList<>();
        for (String date : dates) {
            int[] d = CalendarUtil.strToArray(date);
            if (isIllegal(d)) {
        ...
        mAttrsBean.setSpecifyDate(multiDates);
        return this;
    }
